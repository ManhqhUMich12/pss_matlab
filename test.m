...
for bandwidth = bandwidth_sweep
    ...
    if sweep == 1        %sweep pll gain        
        ...
        para2_.kp_pll = 2*2*pi;
        para2_.ki_pll = para2_.kp_pll * (2*2*pi)/4; 
    ...
    for n = 1:nbus
        ...
        % eigenvalues / poles của thiết bị n (rad/s)
        lam = eig(Gm{n}.A);
    
        % đổi sang Hz giống plot của bạn (MATLAB đang làm pole(Gsys)/2/pi)
        lam_hz = lam / (2*pi);
    
        fprintf('Bus %d (type=%d): eig(A) sample (Hz)=\n', n, type{n});
        disp(lam_hz.');
    end
    Gm = MdlLink(Gm);
    disp(eig(Gm))
